...
It can dramatically improve both the throughput and structures of an application.

From a structural perspective the application looks like many little collaborating computers rather than one big main loop. This is far easier to debug and understand.

In *principle*, each servlet (handles concurrency for web applications) execution lives in its own little world and is decoupled from all the other servlet executions.

This is far less than perfect.
...
- *Concurrency always improves performance*
  - only when there is a lot of wait time that can be shared between multiple threads or multiple processors. Neither situation is trivial.
- *Design does not change when writing concurrent programs.*
  - It can be remarkably different from the design of a single-threaded system.
  - The decoupling of *what* from *when* usually has a huge effect on the structure of the system.
- *Understanding concurrency issues is not important when working with a container such as a Web or EJB container*.
  - You better know how to guard against the issues of concurrent update and deadlock.

Other Important Notes on Concurrency:
...
```
With 2 threads performing this operation, there are 12,870 different possible execution paths for the Java Just-In-Time Compiler with this given generated byte-code. If the type of `lastIdUsed` is changed from `int` to `long`, the number of possible paths increases to 2,704,156. 

Most of those paths generate valid results. The probliem is that *some of them don't*.

#### Concurrency Defense Principles
#### Single Responsiblity Principle
The SRP states that a given method/class/component should have a single reason to change.

Concurrency logic needs to be separated from other production code.
...
One solution to protecting data being updated by multiple threads is to use the `synchronized` keyword to protect a *critical section* in the code that uses the shared object.

It is important to restrict the number of such critical sections. The more places that get updated the more likely:
...
Possible large cost with creating a lot of object copies.
This cost could still be beneficial if the savings in avoiding the intrinsic lock is achieved.

#### Corollary: Threads Should Be Independent as Possible
